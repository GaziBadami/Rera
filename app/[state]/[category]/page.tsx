...
import { ArrowLeft, MapPin, Loader2, Building2, Users, FileText, ChevronRight } from 'lucide-react'
...
export default function CategoryPage({ params }: CategoryPageProps) {
  ...
  // Check if this is a region selection page (Haryana-style)
  const hasSubRegions = 'hasSubRegions' in config && config.hasSubRegions
  const isRegionPage = hasSubRegions && 'regions' in config && category in config.regions

  // If it's a region page, show category selection
  if (isRegionPage && 'regions' in config) {
    const region = config.regions[category as keyof typeof config.regions]
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        ...
        <div className="bg-gradient-to-br from-blue-900 via-blue-800 to-slate-900 dark:from-slate-950 dark:via-blue-950 dark:to-slate-900 text-white">
          <div className="max-w-7xl mx-auto px-6 py-4">
            ...
            <div className="flex items-center space-x-3 mb-1">
              ...
              <h1 className="text-2xl font-bold">{config.name} - {region.name}</h1>
            </div>
            <p className="text-blue-200 text-sm">Select a category</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(region.categories).map(([catKey, categoryInfo]) => {
              const Icon = catKey.includes('project') ? Building2 : Users
              
              return (
                <Link
                  key={catKey}
                  href={`/${state}/${category}/${catKey}`}
                  className="group bg-white dark:bg-slate-800 rounded-xl p-5 shadow-sm hover:shadow-lg transition border border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500"
                >
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-400">
                          View {categoryInfo.label.toLowerCase()}
                        </p>
                      </div>
                    </div>
  ...
  // Otherwise, it's a regular category page - fetch and display data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`/api/data/${state}/${category}?page=${page}&limit=${pageSize}`)
  ...
  }, [state, category, page, pageSize, isRegionPage])
  ...
  if ('hasSubRegions' in config && config.hasSubRegions && config.regions) {
    ...
    categoryInfo = config.categories[category as keyof typeof config.categories]
  }
        ...
        {loading ? (
          ...
          <>
            ...
            <CardGrid
              ...
              state={state}
              type={
                category.includes('agent') || category.includes('registered') ? 'agent' : 
                category.includes('complaint') ? 'complaint' : 
                category.includes('order') || category.includes('judgement') ? 'order' : 
                'project'
              }
              defaultView="grid"
            />
